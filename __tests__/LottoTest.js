...
describe('로또 클래스 생성 테스트', () => {
  ...
  describe('로또 클래스의 getRank 함수 테스트', () => {
    ...
    test.each([
      [[42, 45, 11, 2, 39, 20], [4, 12, 45, 2, 9, 23], 1],
      [[1, 31, 3, 42, 40, 12], [2, 23, 18, 31, 36, 30], 9],
      [[19, 15, 28, 36, 7, 9], [5, 8, 6, 39, 12, 1], 45],
    ])(
      '로또 번호가 일치하지 않거나 2개 이하로 일치하면 null를 반환한다. 나의번호: %p, getRank(%p, %d) => null',
      (numbers, winningNumbers, bonusNumber) => {
        const lotto = new Lotto(numbers);

        expect(lotto.getRank(winningNumbers, bonusNumber)).toEqual(null);
      }
    );
    ...
    test.each([
      ...
      [[1, 2, 19, 32, 38, 39], [1, 3, 19, 33, 37, 40], 2, null],
    ])(
